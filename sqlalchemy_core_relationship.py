...
book_authors = Table('BookAuthor', metadata,
                  ...
                  PrimaryKeyConstraint('BookId', 'AuthorId', name='BookAuthor_pk')
                  )
...
def create_publisher(name):
    stmt = insert(publishers).values(Name=name)
    with engine.connect() as con:
...
def create_author(first, last):
    ...
    with engine.connect() as con:
        ...
        result = con.execute(stmt)
        new_id = result.inserted_primary_key['Id']
        book = select(authors).where(authors.c.Id == new_id)
        return con.execute(book).first()
...
def create_book_details(book):
    stmt = insert(bookdetails).values(
        Id=book.id,
        Cover='https://i.gr-assets.com/images/S/compressed.photo.goodreads.com/books/13093055691/477597.jpg'
    )
    with engine.connect() as con:
        con.execute('pragma foreign_keys=on')
        result = con.execute(stmt)
...
def show_book_and_publisher():
    ...
    stmt = select(publishers, books).select_from(join_publisher_books).\
        where(publishers.c.Id == manning.Id)
    print_result(stmt)

    stmt = select(books.c.Title, books.c.ISBN, books.c.Pages, publishers.c.Name).\
        select_from(join_publisher_books).where(publishers.c.Id == manning.Id)
    print_result(stmt)
...
def show_authors_of_book():
    ...
    join_book_authors = book_authors.join(books).join(authors)
    print(join_book_authors)

    stmt = select(books.c.Title, authors).\
        select_from(join_book_authors).where(book_authors.c.BookId == bookA.Id)
    print_result(stmt)
...
def show_left_outer_join():
    ...
    stmt = select(publishers, books).select_from(join_publisher_books_full)
    print_result(stmt)
